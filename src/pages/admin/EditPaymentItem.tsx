...
export function EditPaymentItem() {
  ...
  const loadData = async () => {
    try {
      ...
      if (paymentItem) {
        setOriginalAmount(paymentItem.amount);
        setExistingIconUrl(paymentItem.icon_url || null);
        setExistingFileUrl(paymentItem.file_url || null);
        setActivitySchedule(paymentItem.schedule || []);
        setFormData({
          category: paymentItem.category,
          title: paymentItem.title,
          description: paymentItem.description || '',
          location: paymentItem.location || 'At school',
          amount: paymentItem.amount.toString(),
          due_date: paymentItem.due_date,
          bank_transfer_enabled: paymentItem.bank_transfer_enabled,
          wipay_enabled: paymentItem.wipay_enabled,
          cash_enabled: paymentItem.cash_enabled,
          whatsapp_notifications: paymentItem.whatsapp_notifications,
          email_notifications: paymentItem.email_notifications,
          eligible_students: [],
          all_students: false,
          is_mandatory: paymentItem.is_mandatory ?? true,
          max_capacity: paymentItem.max_capacity ?? 0,
        });
      }
      ...
      if (assignedStudents) {
        ...
        const isAllStudents = studentIds.length === allStudentIds.length &&
          studentIds.every(id => allStudentIds.includes(id));

        setFormData(prev => ({
          ...prev,
          eligible_students: studentIds,
          all_students: isAllStudents,
        }));
      }
      ...
      if (schedulesData) {
        setSchedules(schedulesData.map(s => ({
          id: s.id,
          send_via_whatsapp: s.send_via_whatsapp,
          send_via_email: s.send_via_email,
          scheduled_datetime: s.scheduled_datetime.slice(0, 16),
          sent: s.sent,
        })));
      }
    } catch (error) {
  ...
  const removeSchedule = (scheduleId: string) => {
    ...
    if (schedule && schedule.sent) {
      alert('Cannot delete a schedule that has already been sent.');
      return;
    }

    if (!scheduleId.startsWith('temp-')) {
      setDeletedScheduleIds([...deletedScheduleIds, scheduleId]);
    }
    setSchedules(schedules.filter((s) => s.id !== scheduleId));
  };

  const updateSchedule = (scheduleId: string, field: keyof Schedule, value: boolean | string) => {
    setSchedules(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedIcon) {
        ...
        if (existingIconUrl) {
          const oldIconName = existingIconUrl.split('/').pop();
          if (oldIconName) {
      ...
      const newStudentIds = formData.all_students
        ? students.map((s) => s.id)
        : formData.eligible_students;

      const studentsToAdd = newStudentIds.filter(
        (sid) => !originalStudentIds.includes(sid)
      );
      ...
      const amountChanged = parseFloat(formData.amount) !== originalAmount;
      if (amountChanged) {
        const studentsToUpdate = originalStudentIds.filter(sid => newStudentIds.includes(sid));

        if (studentsToUpdate.length > 0) {
          const { error: updatePaymentsError } = await supabase
            .from('payments')
            .update({ amount: parseFloat(formData.amount) })
            .eq('payment_item_id', id)
      ...
      const existingSchedules = schedules.filter(s => !s.id.startsWith('temp-'));
      ...
      navigate(`/admin/payment-items/${id}`);
    } catch (error) {
    ...
    <AdminLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate(`/admin/payment-items/${id}`)}
            ...
            {step === 1 && (
              <>
                ...
                <div>
                  ...
                  {!selectedIcon && !existingIconUrl || removeExistingIcon && !selectedIcon ? (
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <Upload className="w-8 h-8 mb-2 text-gray-400" />
                        <p className="mb-2 text-sm text-gray-500">
                          <span className="font-semibold">Click to upload</span> an icon
                        </p>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 text-sm font-medium text-gray-900">
                            <FileImage size={18} />
                            {selectedIcon ? selectedIcon.name : 'Current icon'}
                          </div>
                          {selectedIcon && (
                ...
                <div>
                  ...
                  {!selectedFile && !existingFileUrl || removeExistingFile && !selectedFile ? (
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <Upload className="w-8 h-8 mb-2 text-gray-400" />
                        <p className="mb-2 text-sm text-gray-500">
                ...
                <div className="flex justify-end">
                  ...
                  >
                    {formData.is_mandatory ? 'Next: Update Students' : 'Next: Notifications & Preview'}
                  </button>
                </div>
              </>
            )}
            ...
            {step === 2 && (
              <>
                          ...
                          <div className="space-y-3">
                              ...
                              <div className="flex gap-4">
                                <label className="flex items-center gap-2">
                                  <input
                                    ...
                                    className="w-4 h-4 text-school-red-600 rounded focus:ring-school-red-500 disabled:opacity-50"
                                  />
                            ...
                            <div>
                              ...
                              <input
                                ...
                                disabled={schedule.sent}
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-school-red-500 focus:border-transparent outline-none text-sm disabled:opacity-50 disabled:bg-gray-100"
                                required
                              />
                            </div>
                          </div>
                ...
                <div className="bg-school-cream p-4 rounded-lg">
                  ...
                  {(filePreview || existingFileUrl) && !removeExistingFile && (
                    <div className="mb-4">
                      <button
                        type="button"
                        onClick={() => setShowImageModal(true)}
                ...
                <div className="bg-school-cream p-4 rounded-lg">
                  <h3 className="font-semibold text-school-red-700 mb-2">Note</h3>
                  <p className="text-sm text-gray-700">
                    Adding new students will create payment records for them. Removing students will delete their payment records for this item.
                  </p>
                </div>
      ...
      {showImageModal && (filePreview || existingFileUrl) && (
        <div
